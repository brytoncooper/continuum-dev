...
function wrapFieldComponent(Component: (props: ContinuumNodeProps) => JSX.Element) {
...
function HighlightedControl({ value, onChange, definition, nodeId, children }: ContinuumNodeProps) {
    ...
    return (
      <label style={{ display: 'grid', gap: 8, minWidth: 0 }}>
        <div style={{ display: 'grid', gap: 4 }}>
          <span style={{ fontSize: 12, lineHeight: 1.3, fontWeight: 600, letterSpacing: '0.04em', textTransform: 'uppercase', color: color.textSoft }}>
            {label}
          </span>
          {description ? (
            <span style={{ fontSize: 12, lineHeight: 1.4, fontWeight: 500, color: color.textMuted }}>{description}</span>
          ) : null}
        </div>
        <textarea
          value={text}
          placeholder={nodePlaceholder(definition) ?? `Enter ${label.toLowerCase()}`}
          style={inputLikeStyle({ minHeight: 120, height: 120, resize: 'vertical', ...errorInputStyle() })}
  ...
  if (nodeType === 'select') {
    ...
    const options = readNodeProp<Array<{ value: string; label: string }>>(definition, 'options') ?? [];
    ...
    return (
      <label style={{ display: 'grid', gap: 8, minWidth: 0 }}>
        ...
        <select
          value={selected}
          style={inputLikeStyle({ cursor: 'pointer', paddingRight: 40, ...errorInputStyle() })}
          onChange={(event) => onChange({ value: event.target.value, isDirty: true } as NodeValue)}
        >
  ...
  if (nodeType === 'presentation') {
    return (
      <div style={{ border: '2px solid #c62828', background: '#fff3f3', borderRadius: 12, padding: 12 }}>
        <Presentation definition={definition} nodeId={nodeId} value={value} onChange={onChange}>
          {children}
        </Presentation>
      </div>
    );
