...
6. `usePersistence` serializes snapshots into storage (debounced).
...
### `resolveStorage(persist): Storage | null`
...
### `hydrateOrCreate(storage, key, sessionOptions): { session, wasHydrated }`

Type:
- Internal helper function.

Behavior:
1. If storage exists, read serialized JSON from `storage.getItem(key)`.
2. If data exists:
   - `JSON.parse(raw)`
   - `deserialize(parsed, sessionOptions)`
   - Return hydrated session with `wasHydrated: true`.
3. If parsing/deserialization fails:
   - Remove corrupted storage entry via `storage.removeItem(key)`.
4. Fallback:
   - `createSession(sessionOptions)`, `wasHydrated: false`.
...
4. Invoke `usePersistence(session, storage, storageKey)` to subscribe and persist.
...
Role:
- Snapshot persistence hook used by provider.

### `usePersistence(session, storage, key): void`

Type:
- Exported hook.

Behavior:
1. `useEffect` with deps `[session, storage, key]`.
2. If `storage` is null, early return (no-op).
3. Subscribe to session snapshots via `session.onSnapshot`.
4. On each snapshot:
   - Debounce writes with a 200ms timeout.
   - Serialize with `session.serialize()`.
   - Save with `storage.setItem(key, JSON.stringify(...))`.
   - Swallow storage errors in `try/catch` (silent failure).
5. Cleanup:
   - Clear pending timeout.
   - Unsubscribe from session.

Design notes:
- Debounce reduces storage write frequency during rapid updates.
- Silent catch prevents quota/security errors from crashing UI.
- No explicit flush on unmount; pending write is canceled.
...
- `context.tsx` depends on `@continuum/session`, `types.ts`, `persistence.ts`
...
- `hydrateOrCreate` (`context.tsx`)
- `ContinuumProvider` (`context.tsx`)
- `usePersistence` (`persistence.ts`)
