...
export function createRestoreReviewsFacade(
...
> {
  return {
    ...
    acceptRestoreCandidate(
      detachedKey: string,
      targetNodeId: string,
      scope: Parameters<Session['acceptRestoreCandidate']>[2]
    ) {
      assertNotDestroyed(internal);
    ...
    rejectRestoreReview(
      detachedKey: string,
      scope: Parameters<Session['rejectRestoreReview']>[1]
    ) {
      assertNotDestroyed(internal);
      rejectRestoreReview(internal, detachedKey, scope);
    },
    clearApprovedRestoreTargetsForScope(
      scope: Parameters<Session['clearApprovedRestoreTargetsForScope']>[0]
    ) {
      assertNotDestroyed(internal);
    ...
    updateStateInScope(
      nodeId: string,
      value: NodeValue,
      scope: Parameters<Session['updateStateInScope']>[2]
    ) {
      assertNotDestroyed(internal);
      if (scope.kind === 'live') {
