...
function itemsFor(
  ...
  return (
    (
      result.reconciledState.values[collectionId] as NodeValue<{ items: Array<{ values: Record<string, NodeValue> }> }>
    ).value.items
  );
}
...
describe('cross-level reconciliation', () => {
  describe('category A: semanticKey top-level to collection auto-migration', () => {
    it('moves value into first item when source was removed', () => {
      const priorView = makeView([
        makeNode({ id: 'user_name', type: 'field', semanticKey: 'person.name', dataType: 'string' }),
        makeTargetCollection('tasks', 'assignee'),
      ]);
      const newView = makeView([
        makeTargetCollection('tasks', 'assignee', 'person.name', undefined, 1),
      ], 'view-1', '2.0');
      const priorData = makeData({
        user_name: { value: 'Alice', isDirty: true },
        tasks: { value: { items: [{ values: { 'row/title': { value: 'Task 1' } } }] } },
      });
      ...
      expect(items[0].values['row/assignee']).toEqual({ value: 'Alice', isDirty: true });
      expect(result.reconciledState.values['user_name']).toBeUndefined();
    });

    it('moves value into all existing items', () => {
      const priorView = makeView([
        makeNode({ id: 'status', type: 'field', semanticKey: 'item.status', dataType: 'string' }),
      ...
      expect(items[0].values['row/state']).toEqual({ value: 'archived', isDirty: true });
      expect(items[1].values['row/state']).toEqual({ value: 'archived', isDirty: true });
    });

    it('preserves existing sibling values during migration', () => {
      const priorView = makeView([
        makeNode({ id: 'owner', type: 'field', semanticKey: 'task.owner', dataType: 'string' }),
        makeTargetCollection('tasks', 'assignee'),
      ]);
      const newView = makeView([
      ...
      expect(item.values['row/assignee']).toEqual({ value: 'Riley', isDirty: true });
      expect(item.values['row/title']).toEqual({ value: 'Lift' });
    });

    it('fills seeded minItems and migrates into each seeded item', () => {
      const priorView = makeView([
        makeNode({ id: 'category', type: 'field', semanticKey: 'task.category', dataType: 'string' }),
    ...
    it('does not migrate when source still exists at top level', () => {
      ...
      expect(result.reconciledState.values['owner']).toEqual({ value: 'Kai', isDirty: true });
    });
          ...
          template: makeNode({
            ...
            children: [
              makeNode({
                ...
                template: makeNode({ id: 'child', type: 'field', dataType: 'string' }),
              }),
            ],
          }),
        }),
    ...
    for (const [label, value] of [
      ...
      ['with suggestion metadata', { value: 'D', suggestion: 'E' } as NodeValue],
    ] as const) {
      it(`preserves full node value metadata (${label})`, () => {
        const priorView = makeView([
          makeNode({ id: 'source', type: 'field', semanticKey: 'meta.key', dataType: 'string' }),
        ...
        const result = reconcile(newView, priorView, priorData);
        expect(itemsFor(result, 'tasks')[0].values['row/target']).toEqual(value);
      });
    }

    it('marks collection as migrated in diffs', () => {
      const priorView = makeView([
        makeNode({ id: 'source', type: 'field', semanticKey: 'diff.key', dataType: 'string' }),
        makeTargetCollection('tasks', 'target'),
      ]);
      const newView = makeView([
      ...
      const result = reconcile(newView, priorView, priorData);
      const migrated = result.diffs.find((d) => d.type === 'migrated' && d.nodeId === 'tasks');
  ...
  describe('category B: semanticKey collection to top-level auto-migration', () => {
    it('extracts first item value when moved to top level', () => {
      ...
      const priorData = makeData({
        tasks: {
          value: {
            items: [
              { values: { 'row/assignee': { value: 'Sam', isDirty: true }, 'row/title': { value: 'A' } } },
              { values: { 'row/assignee': { value: 'Tess' }, 'row/title': { value: 'B' } } },
            ],
          },
        },
      });
      ...
      const result = reconcile(newView, priorView, priorData);
      expect(result.reconciledState.values['user_name']).toEqual({ value: 'Sam', isDirty: true });
    });
    ...
    it('does not extract when source node still exists in collection template', () => {
      ...
      const newView = makeView([
        makeNode({ id: 'user_name', type: 'field', semanticKey: 'person.name', dataType: 'string', defaultValue: '' }),
        makeTargetCollection('tasks', 'assignee', 'person.name'),
      ], 'view-1', '2.0');
      const priorData = makeData({
        tasks: {
          value: {
            items: [{ values: { 'row/assignee': { value: 'Sam', isDirty: true } } }],
          },
        },
      });
    ...
    for (const itemCount of [0, 1, 2, 3]) {
      it(`handles ${itemCount} collection items deterministically`, () => {
        const priorView = makeView([makeTargetCollection('tasks', 'assignee', 'person.name')]);
        const newView = makeView([
          makeNode({ id: 'user_name', type: 'field', semanticKey: 'person.name', dataType: 'string', defaultValue: '' }),
          makeNode({
            id: 'tasks',
    ...
    it('does not extract when target type differs', () => {
      ...
      const newView = makeView([
        makeNode({
          ...
          template: makeNode({ id: 'entry', type: 'field', dataType: 'string' }),
        }),
      ], 'view-1', '2.0');
      const priorData = makeData({
        tasks: { value: { items: [{ values: { 'row/assignee': { value: 'Sam' } } }] } },
      });

      const result = reconcile(newView, priorView, priorData);
      expect(result.reconciledState.values['user_name']).not.toEqual({ value: 'Sam' });
    });
  });
  ...
  describe('category C: regular key top-level to collection suggestion-only', () => {
    it('places source as suggestion and preserves target value', () => {
      const priorView = makeView([
        makeNode({ id: 'status', type: 'field', key: 'status', dataType: 'string' }),
      ...
      const priorData = makeData({
        status: { value: 'archived', isDirty: true },
        items: { value: { items: [{ values: { 'row/title': { value: 'A' } } }] } },
      });

      const result = reconcile(newView, priorView, priorData);
      const target = itemsFor(result, 'items')[0].values['row/item_status'] as NodeValue;
    ...
    it('applies suggestion to all existing items', () => {
      ...
      const result = reconcile(newView, priorView, priorData);
      const items = itemsFor(result, 'items');
      expect((items[0].values['row/item_status'] as NodeValue).suggestion).toBe('archived');
      expect((items[1].values['row/item_status'] as NodeValue).suggestion).toBe('archived');
    });
    ...
    it('does not suggest when source still exists at top level', () => {
      ...
      const result = reconcile(newView, priorView, priorData);
      expect(itemsFor(result, 'items')[0].values['row/item_status']).toBeUndefined();
      expect(result.reconciledState.values['status']).toEqual({ value: 'archived', isDirty: true });
    });
    ...
    it('does not suggest when types differ', () => {
      ...
      expect(itemsFor(result, 'items')[0].values['row/counter']).toBeUndefined();
      ...
      it(`keeps existing value "${initial}" while adding suggestion`, () => {
        ...
        const priorData = makeData({
          ...
          items: {
            value: {
              items: [
                { values: { 'row/item_status': { value: initial, isDirty: true } } },
              ],
            },
          },
        });
  ...
  describe('category D: regular key collection to top-level suggestion-only', () => {
    it('places first item value into top-level suggestion', () => {
      ...
      const newView = makeView([
        makeNode({ id: 'user_name', type: 'field', key: 'person.name', dataType: 'string', defaultValue: '' }),
        makeNode({
          id: 'tasks',
          type: 'collection',
          template: makeNode({
      ...
      const priorData = makeData({
        tasks: { value: { items: [{ values: { 'row/assignee': { value: 'A1', isDirty: true } } }] } },
      });
    ...
    it('does not suggest when source template field still exists', () => {
      ...
      const priorData = makeData({
        tasks: { value: { items: [{ values: { 'row/assignee': { value: 'A1' } } }] } },
      });
  ...
  describe('category E: removal gate behavior', () => {
    ...
    for (const t of cases) {
      it(`mode=${t.mode} sourceStays=${t.sourceStays}`, () => {
        const source = makeNode({
          ...
          type: 'field',
          dataType: 'string',
          ...(t.mode === 'semantic' ? { semanticKey: 'gate.key' } : { key: 'gate.key' }),
        });
        const priorView = makeView([source, makeTargetCollection('tasks', 'target')]);
        ...
        const result = reconcile(newView, priorView, priorData);
        const itemValue = itemsFor(result, 'tasks')[0].values['row/target'] as NodeValue | undefined;
        if (!t.expectApplied) {
    ...
    it('counts same-level type replacement as removed for cross-level matching', () => {
      const priorView = makeView([
        makeNode({ id: 'source', type: 'field', semanticKey: 'replace.key', dataType: 'string' }),
        makeTargetCollection('tasks', 'target'),
      ]);
      const newView = makeView([
        makeNode({ id: 'source', type: 'action', semanticKey: 'replace.key', intentId: 'x', label: 'x' }),
      ...
      const result = reconcile(newView, priorView, priorData);
      expect(itemsFor(result, 'tasks')[0].values['row/target']).toEqual({ value: 'Z' });
    });
  ...
  describe('category F: user data protection invariants', () => {
    it('regular key never overwrites dirty item value', () => {
      ...
      const priorData = makeData({
        ...
        items: {
          value: {
            items: [{ values: { 'row/item_status': { value: 'active', isDirty: true } } }],
          },
        },
      });
    ...
    it('regular key never overwrites dirty top-level value on collection-to-top', () => {
      const priorView = makeView([makeTargetCollection('tasks', 'assignee', undefined, 'person.name')]);
      const newView = makeView([
        makeNode({ id: 'user_name', type: 'field', key: 'person.name', dataType: 'string' }),
        makeNode({
          id: 'tasks',
          type: 'collection',
          template: makeNode({
      ...
      const priorData = makeData({
        tasks: { value: { items: [{ values: { 'row/assignee': { value: 'Taylor' } } }] } },
    ...
    it('semanticKey migration preserves dirty metadata', () => {
      const priorView = makeView([
        makeNode({ id: 'source', type: 'field', semanticKey: 'dirty.key', dataType: 'string' }),
        makeTargetCollection('tasks', 'target'),
      ]);
      const newView = makeView([
    ...
    for (const value of ['A', 'B', 'C', 'D', 'E', 'F']) {
      it(`round-trip semanticKey preserves latest user edit (${value})`, () => {
        const v1 = makeView([
          makeNode({ id: 'name', type: 'field', semanticKey: 'person.name', dataType: 'string' }),
          makeTargetCollection('tasks', 'assignee'),
        ], 'view-1', '1.0');
        const v2 = makeView([
          makeTargetCollection('tasks', 'assignee', 'person.name', undefined, 1),
        ...
        const r2 = reconcile(v2, v1, makeData({
          name: { value, isDirty: true },
          tasks: { value: { items: [{ values: {} }] } },
        }));
        const tasksAfter = itemsFor(r2, 'tasks');
        tasksAfter[0].values['row/assignee'] = { value: `${value}-edited`, isDirty: true };
        const priorForV3 = makeData({
          tasks: { value: { items: tasksAfter } },
        });
        const r3 = reconcile(v3, v2, priorForV3);
        expect(r3.reconciledState.values['name']).toEqual({ value: `${value}-edited`, isDirty: true });
      });
    }
  });
  ...
  describe('category G: multi-step scenarios', () => {
    it('regular key round-trip remains suggestion-only in both directions', () => {
      const v1 = makeView([
        makeNode({ id: 'name', type: 'field', key: 'person.name', dataType: 'string' }),
        makeTargetCollection('tasks', 'assignee'),
      ], 'view-1', '1.0');
      const v2 = makeView([
        makeTargetCollection('tasks', 'assignee', undefined, 'person.name', 1),
      ], 'view-1', '2.0');
      const r2 = reconcile(v2, v1, makeData({
      ...
      const v3 = makeView([
        makeNode({ id: 'name', type: 'field', key: 'person.name', dataType: 'string', defaultValue: '' }),
        makeNode({
          id: 'tasks',
          type: 'collection',
          template: makeNode({
      ...
      const r3 = reconcile(v3, v2, makeData({
        tasks: r2.reconciledState.values['tasks'] as NodeValue,
      }));
      expect(r3.reconciledState.values['name']).toEqual({ value: '', suggestion: undefined });
    });

    it('semanticKey takes precedence over key when both are present', () => {
      const priorView = makeView([
        makeNode({ id: 'source', type: 'field', key: 'k1', semanticKey: 's1', dataType: 'string' }),
      ]);
      const newView = makeView([
        makeNode({
          id: 'tasks',
          ...
          template: makeNode({
            id: 'row',
            type: 'group',
            children: [makeNode({ id: 'target', type: 'field', key: 'k1', semanticKey: 's1', dataType: 'string' })],
          }),
        }),
      ], 'view-1', '2.0');
      const priorData = makeData({
      ...
      const result = reconcile(newView, priorView, priorData);
      expect(itemsFor(result, 'tasks')[0].values['row/target']).toEqual({ value: 'X', isDirty: true });
    });
    ...
    for (const push of [1, 2, 3, 4, 5, 6, 7]) {
      it(`preserves determinism through repeated multi-push chain ${push}`, () => {
        ...
        const d3a = reconcile(v3, v2, makeData({ tasks: d2.reconciledState.values['tasks'] as NodeValue }), { clock });
        const d3b = reconcile(v3, v2, makeData({ tasks: d2.reconciledState.values['tasks'] as NodeValue }), { clock });
  ...
  describe('category H: edge cases', () => {
    for (const key of ['', ' ', 'x', 'very.long.semantic.key.path.1', 'very.long.semantic.key.path.2']) {
      it(`handles semanticKey "${key}"`, () => {
        const priorView = makeView([
          makeNode({ id: 'source', type: 'field', semanticKey: key || undefined, dataType: 'string' }),
          makeTargetCollection('tasks', 'target'),
        ]);
        const newView = makeView([
          makeTargetCollection('tasks', 'target', key || undefined, undefined, 1),
        ], 'view-1', '2.0');
        const priorData = makeData({
          source: { value: 'V' },
          tasks: { value: { items: [{ values: {} }] } },
        ...
        if (key.length === 0) {
          ...
          expect(itemsFor(result, 'tasks')[0].values['row/target']).toEqual({ value: 'V' });
        }
      });
    }
  ...
  describe('category I: determinism', () => {
    it('produces identical output for identical inputs across 10 runs', () => {
      const priorView = makeView([
        makeNode({ id: 'source', type: 'field', semanticKey: 'det.key', dataType: 'string' }),
        makeTargetCollection('tasks', 'target'),
      ]);
      const newView = makeView([
      ...
      const outputs = Array.from({ length: 10 }, () => reconcile(newView, priorView, priorData, { clock }));
    ...
    for (const ordering of [0, 1, 2, 3, 4, 5]) {
      it(`node ordering variant ${ordering} does not change result`, () => {
        const source = makeNode({ id: 'source', type: 'field', semanticKey: 'ord.key', dataType: 'string' });
        const coll = makeTargetCollection('tasks', 'target', 'ord.key', undefined, 1);
