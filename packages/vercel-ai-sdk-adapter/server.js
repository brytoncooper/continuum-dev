...
function toPatchOperation(operation) {
  ...
  if (operation.kind === 'insert-node' && operation.node) {
    return {
      op: 'insert-node',
      parentId:
        typeof operation.parentId === 'string' || operation.parentId === null
          ? operation.parentId
          : undefined,
      position:
        operation.position && typeof operation.position === 'object'
          ? operation.position
          : undefined,
      node: operation.node,
    };
  ...
  if (operation.kind === 'move-node' && typeof operation.nodeId === 'string') {
    return {
      op: 'move-node',
...
function writeExecutionEvent(writer, event) {
...
export async function writeContinuumExecutionToUiMessageWriter(args) {
  ...
  while (!next.done) {
    if (next.value.kind !== 'error') {
      writeExecutionEvent(args.writer, next.value);
    }
